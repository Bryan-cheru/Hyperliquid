...
try {
    ...
    # Extract and install (split commands to avoid && issues)
    & plink -batch -pw $Password "$username@$serverIP" "cd /tmp"
    & plink -batch -pw $Password "$username@$serverIP" "unzip -o hyperliquid-deployment.zip"
    ...
    & plink -batch -pw $Password "$username@$serverIP" "cd /var/www/hyperliquid"
    & plink -batch -pw $Password "$username@$serverIP" "npm install --production"
